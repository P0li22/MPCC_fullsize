...
namespace mpcc
{
    ...
    void MPC::setStage(const State &xk, const Input &uk, const State &xk1, const int time_step)
    {
        ...
        stages_[time_step].lin_model = model_.getLinModel(xk_nz, uk, xk1_nz);
                                                           ...
                                                           const std::array<OptVariables, N + 1> &current_solution)
    {
        ...
        for (int i = 0; i <= N; i++)
        {
            updated_x_vec = sqp_mixing_ * (stateToVector(current_solution[i].xk) + stateToVector(last_solution[i].xk)) + (1.0 - sqp_mixing_) * stateToVector(last_solution[i].xk);
            updated_u_vec = sqp_mixing_ * (inputToVector(current_solution[i].uk) + inputToVector(last_solution[i].uk)) + (1.0 - sqp_mixing_) * inputToVector(last_solution[i].uk);
