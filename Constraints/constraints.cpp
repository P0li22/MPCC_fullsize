...
namespace mpcc
{
    ...
    ConstrainsMatrix Constraints::getConstraints(const ArcLengthSpline &track, const State &x, const Input &u) const
    {
        ...
        ConstrainsMatrix constraints_matrix;
        ...
        C_MPC C_constraints_matrix;
        d_MPC dl_constraints_matrix;
        d_MPC du_constraints_matrix;

        C_constraints_matrix.row(si_index.con_track) = track_constraints.C_i;
        dl_constraints_matrix(si_index.con_track) = track_constraints.dl_i;
        du_constraints_matrix(si_index.con_track) = track_constraints.du_i;

        C_constraints_matrix.row(si_index.con_tire) = tire_constraints_rear.C_i;
        dl_constraints_matrix(si_index.con_tire) = tire_constraints_rear.dl_i;
        du_constraints_matrix(si_index.con_tire) = tire_constraints_rear.du_i;

        C_constraints_matrix.row(si_index.con_alpha) = alpha_constraints_front.C_i;
        dl_constraints_matrix(si_index.con_alpha) = alpha_constraints_front.dl_i;
        du_constraints_matrix(si_index.con_alpha) = alpha_constraints_front.du_i;

        // TODO consider the zero order term directly in the functions construdcing the constraints
        dl_constraints_matrix = dl_constraints_matrix - C_constraints_matrix * stateToVector(x);
        du_constraints_matrix = du_constraints_matrix - C_constraints_matrix * stateToVector(x);

        return {C_constraints_matrix, D_MPC::Zero(), dl_constraints_matrix, du_constraints_matrix};
    }
}