...
namespace mpcc
{
    ...
    TireForcesDerivatives Model::getForceFrontDerivatives(const State &x) const
    {
        ...
        // F_fy
        const double dF_y_vx = (param_.Bf * param_.Cf * param_.Df * std::cos(param_.Cf * std::atan(param_.Bf * alpha_f))) / (1. + std::pow(param_.Bf, 2) * std::pow(alpha_f, 2)) * ((param_.lf * r + vy) / (std::pow((param_.lf * r + vy), 2) + std::pow(vx, 2)));
        const double dF_y_vy = (param_.Bf * param_.Cf * param_.Df * std::cos(param_.Cf * std::atan(param_.Bf * alpha_f))) / (1. + std::pow(param_.Bf, 2) * std::pow(alpha_f, 2)) * (-vx / (std::pow((param_.lf * r + vy), 2) + std::pow(vx, 2)));
        const double dF_y_r = (param_.Bf * param_.Cf * param_.Df * std::cos(param_.Cf * std::atan(param_.Bf * alpha_f))) / (1. + std::pow(param_.Bf, 2) * std::pow(alpha_f, 2)) * ((-param_.lf * vx) / (std::pow((param_.lf * r + vy), 2) + std::pow(vx, 2)));
        const double dF_y_D = 0.0;
        const double dF_y_delta = (param_.Bf * param_.Cf * param_.Df * std::cos(param_.Cf * std::atan(param_.Bf * alpha_f))) / (1. + std::pow(param_.Bf, 2) * std::pow(alpha_f, 2));
    ...
    TireForcesDerivatives Model::getForceRearDerivatives(const State &x) const
    {
        ...
        const double dF_x_delta = 0.0;
        // F_ry
        const double dF_y_vx = ((param_.Br * param_.Cr * param_.Dr * std::cos(param_.Cr * std::atan(param_.Br * alpha_r))) / (1. + std::pow(param_.Br, 2) * std::pow(alpha_r, 2))) * (-(param_.lr * r - vy) / (std::pow((-param_.lr * r + vy), 2) + std::pow(vx, 2)));
        const double dF_y_vy = ((param_.Br * param_.Cr * param_.Dr * std::cos(param_.Cr * std::atan(param_.Br * alpha_r))) / (1. + std::pow(param_.Br, 2) * std::pow(alpha_r, 2))) * ((-vx) / (std::pow((-param_.lr * r + vy), 2) + std::pow(vx, 2)));
        const double dF_y_r = ((param_.Br * param_.Cr * param_.Dr * std::cos(param_.Cr * std::atan(param_.Br * alpha_r))) / (1. + std::pow(param_.Br, 2) * std::pow(alpha_r, 2))) * ((param_.lr * vx) / (std::pow((-param_.lr * r + vy), 2) + std::pow(vx, 2)));
        const double dF_y_D = 0.0;
