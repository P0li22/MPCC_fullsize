...
ErrorInfo Cost::getContouringErrorInfo(const ArcLengthSpline &track,const State &x) const
{
...
CostMatrix Cost::getContouringCost(const ArcLengthSpline &track, const State &x,const int k) const
{
    ...
    const ErrorInfo error_info = getContouringErrorInfo(track,x);
...
CostMatrix Cost::getVelrefCost(const ArcLengthSpline &track, const State &x,int k) const
{
    double vel_ref = 5.0;
    vel_ref += x.vx - vel_ref;

    double cost_param_q_vref = 1.0;

    Q_MPC Q_heading_cost = Q_MPC::Zero();
    Q_heading_cost(si_index.phi,si_index.phi) = 2.0*cost_param_q_vref;
    q_MPC q_heading_cost = q_MPC::Zero();
    q_heading_cost(si_index.phi) = -2.0*cost_param_q_vref*vel_ref;
...
CostMatrix Cost::getCost(const ArcLengthSpline &track, const State &x, const Input &u,const int k) const
{
    ...
    // const CostMatrix vref_cost = getVelrefCost(track,x,k);
    ...
    // std::cout << Q_full << std::endl << std::endl;
